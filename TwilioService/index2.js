...
fastify.register(async (fastify) =>{
    fastify.get('/media-stream', {websocket:true},(connection, req)=>{
        ...
        const pythonWs = new WebSocket(`ws://${PYTHON_SERVER_URL}/process-openai`);
        ...
        pythonWs.on('message', (response) => {
            // Send back the response from Python to Twilio
            const twilioResponse = {
                event: 'media',
                streamSid: streamSid,
                media: { payload: Buffer.from(response, 'utf8').toString('base64') }
            };
            connection.send(JSON.stringify(twilioResponse));
        });
                ...
                switch (data.event) {
                    case 'media':
                        if (pythonWs.readyState === WebSocket.OPEN) {
                            // Forward media/text to Python WebSocket server
                            pythonWs.send(JSON.stringify({ media: data.media.payload }));
                        }
                        break;
                    case 'start':
